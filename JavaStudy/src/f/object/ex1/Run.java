...
/*
 	package
 	클래스간의 공간적 충돌이나 접근 방법의 충돌을 막기위해
...
//보통 T_01 이라고 생성하는 class 는 객체생성을 위한 class 가 아니라 main 문을 통해 프로그램 실행을 위한 클래스다.
public class Run {
	/*
	 ...
	 * 5. 추상화 과정
		 ...
		 * 2)그 객체들의 공통적인 속성, 기능들을 추줄할 걸
		 * 3)추출한 것들을 가지고 내가 구현하고자 하는 프로그램의 "실질적인 목적"에 맞춰 불필요한 속성, 기능을 제거할 것
		 * 4)최종적으로 추려진 속성들을 어떤 자료형으로, 어떤 변수명으로 사용할 것인지 정의
		 * 
		 * EX) 학생 관리 프로그램
		 * 1) 학생관련 객체(최경채훈련생, 최용준훈련생, 안재휘훈련생등등등...)
		 * 2) 공통적인 속성 및 기능(이름, 나이, 주소, 전화번호, 키, 몸무게, 머리색, 발사이즈, 달리기속도, 먹는다, 잔다, 공부한다, 휴식한다)
	 ...
	 *    최경채 라는 훈련생 하나 만들기 -> String name1 = "최경채"; int age 20; double height = 185.0;
	 *    안재휘 라는 훈련생 하나 만들기 -> String name2 = "최경채"; int age2 19; double height2 = 193;
	 *    
	 *    2.배열로 프로그래밍 한다면
	 *    	배열 : 하나의 자료형으로 여려개의 값들을 보관할 수 있음
	 *    	이름을 보관하는 배열 String[] name = {"최경채", "안재휘" ...}
	 *    	나이를 보관하는 배열 int [] age = {20,19...}
	 *    
	 *    3. 그래서 나온 개념 -> 구조체
	 *    구조체: 여러개의 자료형의 여러개의 값들을 보관할 수 있다.
	 *    String 값도 보관하고 int 값도 보관하고 동시에 double 값도 함께 보관하는 자료형을 직접 만든다고 생각 -> 나만의 자료형
	 *    Struct human{
	 *    	String name;
	 *    	int age;
	 *    	double height;
	 ...
	 *   
	 *   *캡슐화 : 추상화를 통해서 정의된 속성들과 기능들을 하나로 묶어 관리하는 개념
	 *   		클래스에서 가장 중요한 목적인 "데이터의 접근제한(정보은닉)"을 원칙으로
	 *   		외부로부터의 "데이터접근을 막고"
	 *   		대신에 "데이터를 간접적으로 처리(값을 변경하거나, 가져오는 것을)" 할 메소드들을 클래스 내부에 작성하여 관리하는 것
	 *   		다양한 기능을 묶어 하나의 큰기능을 만들어주는 것
		 */
