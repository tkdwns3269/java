...
//		
		 ...
		 * 주고값을 담고 있는 변수는 참조 변수(레퍼런스 변수)라고 표현
		 * 
		...
		//내가 각 인덱스의 초기화를 하지 안하도 값들이 담겨있다.
		//Heap 이라는 메모리공간은 절대 빈공간을 허용하지 않는다.
		...
		/*
		 * null 을 가지고 있는 참조변수에 접근하여 다른 정보를 구하고자 하면 어떻게 될까?
		 * 항상 오류가 발생한다.
		 * 
		...
		arr = null;
		//배열을 강제로 삭제시키고자 한다면 => 연결고리를 귾어주면 된다. (null 대입)
		
		int[] arr2 = {1,1,1,1,1};
		int[] arr3 = {1,1,1,1,1};
		System.out.println(arr2 == arr3); // false => 각 참조변수는 ==을 통해서 비교시 주소값을 비교하기 때문에 
		
		arr3 = arr2; //주소값이 복사되어 서로 같아짐
		System.out.println(arr2== arr3); // true
		
		arr3[0]=100;
		System.out.println(arr2[0]); //arr2 와 arr3의 주소값이 같아졌으므로 arr3[0]에 넣은 100이 arr2[0]에 넣은것과 같다/
	}
