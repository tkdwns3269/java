...
public class MapRun {
	public static void main(String[] args) {
		...
		System.out.println(hm);
		//동일한 키값으로 다시 추가하는 경우 value값이 덮어씌어진다.!! (중복된 키값이 공존할 수 없음 키값이 식별자 역할을 함)
		...
		hm.replace("먹태깡", new Snack("아주 매운맛",800)); //만약 해당 키값이 없으면 수정 x
		...
		while(it.hasNext()) {
			Entry entry = (Entry)it.next(); //Entry 객체안에 key, value 값이 다 담겨져있다.
